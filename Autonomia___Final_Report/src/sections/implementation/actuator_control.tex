...
solver runs on the Pi (Section~8.7). The firmware
...
opens the Dynamixel bus at 1~Mbps using the Dynamixel2Arduino library,
...
range for that motor (Section~8.7.4). If any value
...
The gripper motor (M5, XM430-W350) is the same model as four of the
arm motors and lives on the same TTL bus. The firmware treats it
...
\subsubsection*{Current limit at the scan pose}
...
register. The host uses this command only for the elbow motor (M3),
and only at the scan pose, as described in Section~8.4.
When the arm parks at the scan pose, the host sends
\texttt{LIMIT m3 300}. The default value of 1200~mA is restored
before the next motion. We set the limit through the motor's own
control table, not through firmware logic. That way the protection
stays in effect even if the firmware crashes between setting the
limit and sending the next motion. A motor that silently disregards
the limit could overheat. A host that silently disregards a fault is
caught the next time it sends a command.
...
Dynamixel XM430-W350 exposes its present position, present load,
