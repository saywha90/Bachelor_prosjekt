...
To address this, the firmware applies a reduced current limit to 
the elbow motor whenever the arm is parked at \texttt{SCAN\_POSE}. 
The current limit is set to 300~mA, which is sufficient to hold 
the arm in position under nominal load but reduces the steady-state 
heat generation in the motor. When the arm leaves \texttt{SCAN\_POSE} 
to perform a pick-and-place operation, the current limit is 
restored to its default value to provide full torque during 
motion. The transition between the two limits is handled 
automatically by the host processor as part of the state machine 
described in Section~7.1.

This protection is a runtime mitigation rather than a complete 
solution. If the reduced current limit is set too low, the motor 
may not produce sufficient holding torque under unexpected load 
and the arm may sag, which would invalidate the camera 
calibration described in Section~8.8. The 300~mA value 
was therefore selected empirically as the lowest current at which 
the arm remained stable at \texttt{SCAN\_POSE} under the project's 
normal operating conditions, and present-current telemetry from 
the motor is logged at runtime so that operators can detect any 
drift in the holding behaviour.