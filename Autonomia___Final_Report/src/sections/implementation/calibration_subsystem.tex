\hmsubsection{Calibration Subsystem}{UMA}{}
...
sag calibration (Section~8.8.1), touch calibration 
(Section~8.8.2), and scan-pose calibration (Section~8.8.3).

\hmsubsubsection{Sag Calibration}{UMA}{}

The sag calibration procedure determines the coefficients of the 
empirical droop model described in Section~8.7.3. The procedure 
...
(Section~8.8.2), which bypass the polynomial correction via the 
...
\hmsubsubsection{Touch Calibration}{UMA}{}
...
Section~8.7.5 converts these values into a workspace position. 
...
\hmsubsubsection{Scan Pose Calibration}{UMA}{}
...
sub-degree positional repeatability under nominal load, but 
the elbow motor must continuously resist the gravitational 
torque produced by the forearm and camera assembly while the 
arm is parked at the scan pose. As described in Section~8.4, 
the elbow motor's holding current is reduced when the arm is 
stationary at the scan pose to prevent thermal overload, but 
this reduction also lowers the holding torque available to 
resist gravity. If the holding current is reduced too far, 
the elbow may slowly droop while the arm is parked, 
displacing the camera and invalidating the perspective 
transformation. The current limit is therefore selected as 
the smallest value at which the arm remains stable over 
extended observation periods, and the runtime monitors the 
present current of the elbow motor to detect drift in the 
holding behaviour.