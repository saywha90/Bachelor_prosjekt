...
| Person | Rolle | Tema |
|--------|-------|------|
| **U** | Kinematikk, hardware & verifisering | IK-solver, simulering, 3D-visualisering, FK-tester, touch-/sag-/scan-kalibrering, hardware-valg, motor-diagnostikk, termal-beskyttelse |
| **F** | Firmware & system-integrasjon | OpenRB-bro, hovedløkke, konfigurasjonsarkitektur, motor-kalibrering |
...
| `src/simulation/bin_safety.py` | Sikkerhetslogikk for bin-operasjoner og validering av trygge dropp-/plasseringstilstander i simulering. |
| `src/simulation/mock_serial.py` | Falsk seriell-port for testing av `main.py` uten fysisk hardware. |
| `src/simulation/route_demo.py` | Simulert rutedemo for kalibrerte bin-ruter og systemflyt uten fysisk hardware. |
| `src/simulation/sample_route_calibration.json` | Eksempeldata for rutedemo og kalibrerte bin-posisjoner i simulering. |
...
| `src/calibration/02b_claw_grip_test.py` | Klo-grep-test for verifisering av fysisk gripestyrke og adaptiv gripeoppførsel på armen. |
...
| `tests/test_simulation_route_demo.py` | Tester simulert rutedemo og at rute-/kalibreringsdata tolkes riktig. |
| `tests/conftest.py` | Pytest-fixtures for IK-testene (`arm`, `arm_no_sag`, `home_position`, `bin_positions`). |
| `tests/test_adaptive_grip.py` | Tester adaptiv gripelogikk og at klo-verifisering reagerer riktig på ulike gripe-/last-scenarier. |
| `tests/test_touch_calibration_safety.py` | Tester sikkerhetsgrenser og feilhåndtering rundt touch-kalibrering. |
...
- Simulering — mock-seriell, rutedemo, bin-safety og kalibreringsdata uten fysisk hardware
...
| `src/calibration/02b_claw_grip_test.py` | Aktiv diagnostikk for adaptiv klo-/gripeoppførsel etter at `CLAW_OPEN_POS` og `CLAW_CLOSED_POS` er satt manuelt i `src/config/arm.py`. |
...
| `tests/test_arm_config.py` | Tester arm-konfigurasjon, bin-oppslag og hjelpefunksjonene som binder hovedløkken til fysisk geometri. |
| `tests/test_bin_calibration_route_tool.py` | Tester bin-kalibreringens ruteverktøy og integrasjonen mot kalibrerte bin-posisjoner. |
...
- Motor-kalibrering (joints, manuell klo-konfigurasjon i `src/config/arm.py`)
...
| `src/vision/models/.gitignore` | Holder modellmappen sporbar uten å sjekke inn genererte modellartefakter utover eksplisitt valgte modeller. |
| `src/vision/models/ball_color_classifier.pkl` | Trenet fargeklassifikator brukt av vision-pipelinen. |
...
| `tests/test_vision_scan_pose.py` | Tester vision-relatert SCAN_POSE-oppsett og samspillet mellom kamerakonfigurasjon og scan-posisjon. |
...
| `src/calibration/README.md` | Oversikt over alle kalibreringsstegene 02–11 (alle tre eier ulike steg). |
