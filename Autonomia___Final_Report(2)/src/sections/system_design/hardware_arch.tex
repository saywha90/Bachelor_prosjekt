\hmsubsection{Hardware Architecture}{UMA}{OAH}\label{sec:hardware-architecture}
...
turn. The first subsection describes the components of the vision system. 
The following subsections describe the power architecture, including 
...
Subsequent subsections describe the selection and sizing of the 
Dynamixel servos for each joint, and Section~\ref{sec:controller-selection} 
describes the selection of the OpenRB-150 as the embedded motor 
controller.



\hmsubsubsection{Hardware Components for the Vision System}{OAH}{UMA}
...
\hmsubsubsection{Power Architecture}{UMA}{OAH}\label{sec:power-architecture}
...
and required unstable workarounds during testing \cite{sortify}. The 
...
\hmsubsubsection{Theoretical Current Demand}{UMA}{OAH}
...
iteration, is discussed in Section~\ref{sec:motor-family}.
...
\begin{table}[htbp]
    ...
    \begin{tabular}{|l|l|c|}
        ...
        \hline
        Base (M1)     & XM430-W210-T & 2.3 \\
        \hline
        Shoulder (M2) & XM540-W270-T & 4.4 \\
        \hline
        Elbow (M3)    & XM430-W210-T & 2.3 \\
        \hline
        Wrist (M4)    & XL430-W250-T & 1.3 \\
        \hline
        Claw (M5)     & XM430-W210-T & 2.3 \\
        \hline
...
practice, since only a subset of motors are normally accelerating 
at any given time and the payload is light (a 50~mm ball weighing 
approximately 20~g). The arm was tested successfully with a 
6~A power supply, indicating that the typical operating current 
remained below the simultaneous-stall estimate during the tested 
use cases. The stall current is relevant primarily for sizing the 
supply to tolerate transient peaks and short-duration overloads.

\hmsubsubsection{Selection of the Power Supply}{UMA}{OAH}\label{sec:power-supply-selection}
A dedicated 12~V battery rated at 10~A was initially considered because 
it would cover expected operating current and provide transient margin, 
although it would not cover the full simultaneous-stall current of 
12.6~A. It would also allow the system to operate independently of 
mains power, which is relevant for the future integration of the arm 
...
the electrical design. Second, successful testing with the 6~A supply 
showed that the tested operating cases did not require the full 
simultaneous-stall current. Third, the mobile-platform integration is 
outside the scope of the current project, and a battery solution can 
be introduced in a later iteration without affecting the rest of the 
architecture.

\hmsubsubsection{Distribution Topology}{UMA}{OAH}\label{sec:power-distribution}
...
servo before reaching the next \cite{sortify}. This arrangement was 
...
daisy-chained. This arrangement reduces current concentration 
compared with a single five-servo chain, although the exact safety 
margin depends on verified connector and cable ratings. The data line 
follows the same physical wiring, as each 3-pin Dynamixel TTL 
connection carries motor supply voltage, ground, and a single-ended 
half-duplex data line on the same connector.
...
while reducing current concentration at the first connector of either 
...
\begin{figure}[H]
    ...
    branch leaves the OpenRB-150 through a separate cable, reducing 
    current concentration compared with a single five-servo chain. The 
    branch currents shown are the worst-case stall totals for each 
    branch. The exact safety margin depends on verified connector and 
    cable ratings.}
    \label{fig:power_topology}
\end{figure}

\hmsubsubsection{Motor Selection and Joint Sizing}{UMA}{OAH}
...
\hmsubsubsection{Choice of Motor Family}{UMA}{OAH}\label{sec:motor-family}
...
an Arduino Mega \cite{sortify}. This arrangement required two 
distinct control paths, two separate power supplies, and 
additional driver circuitry. In the present design, the analogue 
gripper servo was replaced with a Dynamixel XM430-W210-T. This 
...
\hmsubsubsection{Joint-Specific Motor Sizing}{UMA}{OAH}
...
\begin{table}[htbp]
    ...
    \begin{tabular}{|l|l|c|c|c|}
        ...
        \textbf{Mass} &
        \textbf{Cost} \\
         & & \textbf{(N$\cdot$m at 12~V)} & \textbf{(g)} & \textbf{(rel.)} \\
        \hline
        Base (M1)     & XM430-W210-T & 3.0 & 82  & Medium \\
        \hline
        Shoulder (M2) & XM540-W270-T & 10.6 & 165 & High \\
        \hline
        Elbow (M3)    & XM430-W210-T & 3.0 & 82  & Medium \\
        \hline
        Wrist (M4)    & XL430-W250-T & 1.5 & 57.2 & Low \\
        \hline
        Claw (M5)     & XM430-W210-T & 3.0 & 82  & Medium \\
        \hline
    \end{tabular}
...
all five joints. The XM540-W270-T, with a stall torque of 
10.6~N$\cdot$m at 12~V, was selected for this joint to provide 
sufficient margin above the worst-case loaded condition.

The elbow joint (M3) supports the mass of the wrist, gripper, and 
payload over the length of the forearm. The XM430-W210-T was chosen 
as a compromise between torque capacity (3.0~N$\cdot$m) and weight 
...
acceleration during base motion. The XM430-W210-T was chosen for 
...
operates close to its rotation axis. The XL430-W250-T, with a stall 
torque of 1.5~N$\cdot$m at 12~V, was selected for this joint. Its lower 
mass (57.2~g) and lower cost compared with the XM430 reduce both 
...
downstream joints. The XM430-W210-T was selected for the gripper, 
matching the base and elbow motors and allowing these three 
actuators to share the same spare-part inventory.

\hmsubsubsection{Link Lengths}{UMA}{OAH}\label{sec:link-lengths}
...
motor within the torque envelope of the XM540-W270-T selected in 
the previous paragraph.

\hmsubsubsection{Embedded Motor Controller Selection}{UMA}{}\label{sec:controller-selection}
...
\cite{sortify}. This combination was reviewed at the start of 
...
servo with a Dynamixel motor (Section~\ref{sec:motor-family}) removed the immediate
...
selected in Section~\ref{sec:power-supply-selection}. Second, the specific board acquired
...
Section~\ref{sec:actuator-control}, and the communication 
protocol between the host processor and the OpenRB-150 is 
described in Section~\ref{sec:comm}.
