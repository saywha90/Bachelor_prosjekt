\hmsubsection{Image Processing Pipeline}{OAH}{UMA}\label{sec:image-processing}

This section describes the image processing pipeline responsible for detecting
and classifying red and blue balls in each frame captured by the OAK-D~S2 RGB
...
\hmsubsubsection{Image Acquisition}{OAH}{}\label{sec:acquisition}

Image acquisition is handled by the OAKCamera class, which wraps the
OAK-D~S2 RGB camera through the DepthAI v3~API\@. The camera is equipped
...
\hmsubsubsection{Adaptive Lighting Compensation}{OAH}{UMA}\label{sec:adaptive-lighting}
...
\hmsubsubsection{Image Preprocessing}{OAH}{UMA}
...
rejected, as the resulting ball radius of approximately 7--8 pixels caused the
...
kernel. Each output pixel (highlighted) is the weighted sum of its neighbourhood
divided by 273. Pixels closer to the centre receive higher weights, suppressing
