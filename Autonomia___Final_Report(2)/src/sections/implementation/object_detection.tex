\hmsubsection{Object Detection}{OAH}{UMA}\label{sec:object-detection}
...
Red requires four ranges due to the circular nature of the hue axis in OpenCV,
where hue values are represented in the interval $[0, 179]$. Red occupies both
ends of this interval, wrapping around at the boundary, requiring one range at
each end. Two ranges target normally illuminated red surfaces:
$H \in [160, 179]$ and $H \in [0, 10]$, both with $S \in [120, 255]$ and
$V \in [40, 255]$. Two further ranges target dark red surfaces in shadow or under
low ambient light: $H \in [160, 179]$ and $H \in [0, 10]$, both with
$S \in [80, 255]$ and $V \in [20, 180]$. The lower saturation and value floors in
the shadow ranges allow detection of the same physical ball when it is partially
occluded or shaded by the robot arm.
...
\end{tikzpicture}
\caption[HSV hue axis and resulting binary masks for red and blue detection.]{Left:
the HSV hue axis ($0$--$179$). Red occupies both ends of the axis and requires four
ranges in total: two for normally illuminated surfaces ($S \geq 120$) and two for
surfaces in shadow ($S \geq 80$), one range at each end of the wraparound. Blue
occupies a continuous region covered by three ranges over $H \in [85, 135]$. Right:
the resulting binary masks isolate the relevant colour regions.}
\label{fig:hsv_mask}
\end{figure}
...
\end{tikzpicture}
\caption[Hough Circle Transform in three steps.]{Edge pixels vote in an accumulator
for all circles that could have produced them. A peak in the accumulator (red cell)
corresponds to a circle centre and radius supported by many edge pixels.}
\label{fig:hough}
\end{figure}
...
\hmsubsubsection{Ensemble Detection and Confidence Scoring}{OAH}{}\label{sec:ensemble}
...
\hmsubsubsection{SVM Colour Verification}{OAH}{}\label{sec:svm}
...
\hmsubsubsection{Object Tracking}{OAH}{}\label{sec:tracking}
