\hmsubsection{Inverse Kinematics}{UMA}{}\label{sec:ik-solver}
...
choice of solution method (Section~\ref{sec:ik-choice}), the mathematical 
derivation of the solver (Section~\ref{sec:ik-derivation}), the empirical sag 
compensation applied as post-processing (Section~\ref{sec:sag}), the 
joint limit and reachability constraints (Section~\ref{sec:joint-limits}), and 
the forward kinematics used for validation and limp-mode 
operation (Section~\ref{sec:fk-validation}).

\hmsubsubsection{Choice of Solution Method}{UMA}{}\label{sec:ik-choice}
...
\begin{table}[H]
    ...
    \label{tab:ik_methods}
\end{table}

The selection of the geometric approach is justified by three 
properties of the arm. First, the system uses a 
four-degree-of-freedom arm with a fifth motor for the gripper, 
which is below the threshold at which numerical methods become 
necessary. Standard textbook approaches 
...
\hmsubsubsection{Solution Derivation}{UMA}{}\label{sec:ik-derivation}

The arm's kinematic structure is shown in Figure~\ref{fig:ik_geometry}. The base joint 
...
USB serial connection described in Section~\ref{sec:comm}. The gripper 
...
\hmsubsubsection{Sag Compensation}{UMA}{}\label{sec:sag}

The geometric solution described in Section~\ref{sec:ik-derivation} assumes that the 
...
Section~\ref{sec:sag-calibration}.

The measurements obtained during calibration of the present arm
are summarised in Table~\ref{tab:sag_calibration}. Three reach distances (24, 30, and
...
$r$ is the horizontal reach in centimetres. Figure~\ref{fig:sag_calibration} shows the 
...
calibration procedure described in Section~\ref{sec:touch-cal}. Because these 
...
(Section~\ref{sec:touch-cal}), which provide a direct empirical correction at 
...
\hmsubsubsection{Joint Limits and Reach Clamping}{UMA}{}\label{sec:joint-limits}
...
empirically for the present arm are summarised in Table~\ref{tab:joint_limits}.
...
\hmsubsubsection{Forward Kinematics and Validation}{UMA}{}\label{sec:fk-validation}
...
Section~\ref{sec:ik-derivation}. Motor step values are converted back to joint 
angles using the inverse of the conversion in Equation~\eqref{eq:dynamixel_steps}, and 
...
in Section~\ref{sec:calibration} and provides a more accurate alternative to manual 
